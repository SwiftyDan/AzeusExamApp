...
//        let vc = SearchVC(text:"", searchCode: String(10))
    ...
    private func getRecent() {
        ...
        guard let data = savedRecentData else {return}
...
extension RecentVC: RecentViewDelegate {
    func recentViewDidSelect(_ view: RecentView, data: SavedData) {
      
        let vc = DetailsViewController(nil, pdfData: nil, 1, data)
            show(vc)

...
class RecentCell: UICollectionViewCell {
    ...
    func configure(_ data: SavedData? = nil) {
        ...
        if let imageUrl = data?.images, let url = URL(string: imageUrl) {
         
            imageView.kf.indicatorType = .activity
           
            imageView.kf.setImage(with: url, placeholder: UIImage(named: "Home-fill"), options: [.memoryCacheExpiration(.days(3))], progressBlock: nil, completionHandler: nil)
